...
if spell_class == "explosion" {
	if aggregate_elemental_interaction(elements, other.elements, global.elemental_data) >= 0 {
		instance_destroy(other);
	}
}

if spell_class == "projectile" or spell_class == "wallCrawl"{
	var agg = aggregate_elemental_interaction(elements, other.elements, global.elemental_data);
	if agg > 0 {
		instance_destroy(other);
	} else if agg < 0 {
		if random(100) < 10 {
...
if other.solid {
	...
	if spell_class == "projectile" {
		...
		if variable_instance_exists(self, "impact_payload") {
			...
			for (var i=0; i<min(impact_count, impact_reps*impact_count); i++) {
